...
#"BTC-EUR""BTC-USD"
check_ticker="BTC-GBP"
PAIRS = []  # Array containing all pairs
E_GDAX = Exchange("Coinbase", [check_ticker], 0)
...
def prepare_data(ticker, exchange):
    ...
    for index, row in shape_bid[combined].iterrows():
        ...
        secret_sell.append(""+str(row['min_Price'])+"--"+str(row['max_Price'])+": "+str(row['count'])+" buy orders * ₿"+str(row['unique'])+" = ₿"+str(row["volume"])+" ($"+str(format_number(tot_price1))+")\n")
    ...
    for index, row in shape_ask[combined].iterrows():
        ...
        secret_buy.append(""+str(row['min_Price'])+"--"+str(row['max_Price'])+": "+str(row['count'])+" sell orders * ₿"+str(row['unique'])+" = ₿"+str(row["volume"])+" ($"+str(format_number(tot_price))+")\n")
    ...
    result = {
        ...
        'layout': go.Layout(
            ...
            title=("The present market price of {} on {} is: {}{} at {}".format(ticker, exchange, symbol,str(marketPrice[combined]),timeStamps[combined])),
            # ('The present market price of BTC-USD on GDAX is: $43429.0 at 23:03:44',)
...
#                   ****Information about market****
    ...
    total_buy_order=""+total_buy_order[16:20]+" buy orders valued at ₿"+total_buy_order[0:6]+", "+total_buy_order[-1]+total_buy_order[-9:-1]+"--"+total_buy_order[-1]+str(current_price)+"\n"
    #**sell**#
    sell_number=(ob_ask['text'][58])
    sell_number=float(sell_number[0:4])
    total_sell_order= str(ob_ask['text'][58])
    total_sell_order=""+total_sell_order[16:20]+" sell orders valued at ₿"+total_sell_order[0:6]+", "+total_sell_order[-1]+str(current_price)+"--"+total_sell_order[-1]+total_sell_order[-9:-1]+"\n"
    ...
    #     if i>market_price:
    #         add_1="₿"+str(data["volume"][i])+" one sell order at "+str(i)+" ("+format_number(str(float(data["volume"][i])*market_price))+")\n"
    #         uniqe_buy.append(add_1)
    #     else:
    #         add_2="₿"+str(data["volume"][i])+" one buy order at "+str(i)+" ("+format_number(str(float(data["volume"][i])*market_price))+")\n"
    ...
    #****Save Template File and Html****#
    if not os.path.exists("./templates"):
        os.mkdir("./templates")
    fig = dict(data = result["data"], layout = result["layout"])
    plotly.offline.plot(fig, filename='./templates/'+pair.ticker+'.html',auto_open=False)

    # ****Save Html to image ****#
    # if not os.path.exists("./tmp"):
    #     os.mkdir("./tmp")
    # fig = dict(data=result["data"], layout=result["layout"])
    # plotly.offline.plot(fig, filename='./tmp/' + pair.ticker + '.html', auto_open=False)
    # hti = Html2Image()
    # hti.output_path = './tmp/'
    #
    # with open('tmp/'+pair.ticker+'.html') as f:
    #     hti.screenshot(f.read(), save_as=pair.ticker+".png")
    print("#######################Done##############################")

    #       #-----***Google Sheet***-----#
    #
    # sheet_acount = gspread.service_account(filename="YOUR.json")
    # sheet_name = sheet_acount.open("NAME OF SHEET")
    # sheet_page = sheet_name.worksheet("2022")
    # sheet_page.update_acell("A2",sheet_info)

        #-----****message Telegram****-----#
    #message1=total_sell_order+total_buy_order+okk+uniqe_buy+secret_buy+"------------\n"+uniqe_sell+secret_sell
    # url = requests.get('https://api.telegram.org/botYOUR:'
    #                    "YOUR/sendMessage?chat_id="
    #                    '-YOUR&text='+message)
    #
    # # #   ****Send Image Telagram****
    # file = "./tmp/ooo.png"
    # files ={'photo': open(file, 'rb')}
    # message =('https://api.telegram.org/botYOUR:'
    #                    "YOUR/sendPhoto?chat_id="
    #                    '-5YOUR')
    ...
    @app.route('/')
    def html():
        return render_template(pair.ticker+'.html')
    @app.route('/info')
    def info():
        return "<p>"+okk+"</p>"

    return app.run(host='0.0.0.0', port=8050)
